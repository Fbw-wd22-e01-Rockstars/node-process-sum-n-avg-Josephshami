...
const input = process.argv.slice(3).map(x => Number(x))
const notNumber = process.argv.slice(3).filter(x => isNaN(Number(x)) )
operation[0] = notNumber.length > 0 ? "Error" : operation[0];
 switch (operation[0]) {
     case 'sum':
     ...
     case 'avg':
        ...
        break;
        case 'Error' :
      console.log (`Sorry, the argument '${notNumber[0]}' is not a number, please try again`)
      break;
